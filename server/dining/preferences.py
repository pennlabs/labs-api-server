    ...
    if venues:
        ...
        for venue_id in venue_ids:
            dining_preference = DiningPreference(
                user_id=user.id, account=account_id, venue_id=venue_id
            )
...
def get_dining_preferences():
    ...
    preferences = (
        sqldb.session.query(DiningPreference.venue_id, func.count(DiningPreference.venue_id))
        .filter_by(user_id=user.id)
        .group_by(DiningPreference.venue_id)
        .all()
    )
