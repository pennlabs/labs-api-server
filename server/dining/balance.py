...
def get_dining_balance():
    ...
    dining_balance = (
        DiningBalance.query.filter_by(account_id=account.id)
        .order_by(DiningBalance.created_at.desc())
        .first()
    )
    ...
    if dining_balance:
        ...
        timestamp = created_at.strftime("%Y-%m-%dT%H:%M:%S") + "-{}".format(
            wharton.get_dst_gmt_timezone()
        )
...
def get_average_balances_by_day():
    ...
    if start_date_str and end_date_str:
        ...
        dining_balance = DiningBalance.query.filter_by(account_id=g.account.id).filter(
            DiningBalance.created_at >= start_date, DiningBalance.created_at <= end_date
        )
    else:
...
def get_dining_projection():
    ...
    if dining_balance:
        ...
        swipes_left = (
            df.tail(1).iloc[0]["swipes"] - (swipes_per_day * day_difference) if num_swipes else 0.0
        )
        dollars_left = (
