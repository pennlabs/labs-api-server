...
def get_nso_events():
    ...
    filtered = [
        i if "<pubDate" not in i else "<pubDate>" + parseDate(i) + " EST</pubDate>" for i in split
    ]
    filtered = [
        i if ("<title" not in i or "NSO Event Calendar" in i) else changeTitle(i) for i in filtered
    ]
    output = "\n".join(filtered)
