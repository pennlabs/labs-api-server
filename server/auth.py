...
def anonymous_auth(f):
    ...
    def _anonymous_auth(*args, **kwargs):
        ...
        if not device_key or not password_hash or not data_type:
            return jsonify({'error': 'Missing header X-Device-Key or X-Password-Hash or X-Data-Type.'}), 400

        anonymous_id = AnonymousID.query.filter(AnonymousID.type == data_type) \
                                        .filter(or_(AnonymousID.password_hash == password_hash,
        ...
        if anonymous_id:
            ...
            anonymous_id = AnonymousID(device_key=device_key, password_hash=password_hash, type=data_type)
