...
def add_courses_anonymously(anonymous_id, courses):
    ...
    for term in terms:
        ...
        query = (
            sqldb.session.query(CourseAnonymousID, Course)
            .join(Course)
...
def add_courses_to_account(account, courses):
    ...
    for term in terms:
        query = (
            sqldb.session.query(CourseAccount, Course)
            .join(Course)
    ...
    for json in json_array:
        ...
        if start_date_str and end_date_str:
            ...
            term_start_end_dates = (
                sqldb.session.query(
                    Course.start_date, Course.end_date, func.count(Course.id).label("count")
                )
                .filter_by(term=term)
        ...
        if course:
            ...
            if (
                ...
                or course.end_date != end_date
                or course.building != building
                or (course.room != room)
            ):
                course.start_date = start_date
...
def get_courses(account, day=None, weekday=None, include_extra_meeting_times=False):
    ...
    courses = (
        []
    )  # All Courses. If weekday is not None, only ourses that do not have an extra_meetings_flag
    ...
    instructor_query = (
        sqldb.session.query(Course, CourseInstructor)
