...
def fastest_route():
    ...
    for route in route_data:
        ...
        for stop in stops:
            distFrom = haversine(
                lonFrom, latFrom, float(stop["Longitude"]), float(stop["Latitude"])
            )
        ...
        if fromStop and toStop and fromStop["order"] < toStop["order"]:
            ...
            def add_path_points(loc, stop):
                if stop["order"] == fromStop["order"]:
                    return loc + [stop]
                else:
                    path_to = stop["path_to"]
                    del stop["path_to"]
                    return loc + path_to + [stop]
    ...
    if len(possible_routes) == 0:
        return jsonify(
            {"Error": "We couldn't find a helpful Penn Transit route for those locations."}
        )
    # Choose the route with the minimum total walking distance
    final_route = min(
        possible_routes, key=lambda x: x["walkingDistanceBefore"] + x["walkingDistanceAfter"]
    )
...
def populate_stop_info(stops):
    ...
    try:
        stop_dict = {
            stop["BusStopName"]: stop for stop in stops["result_data"] if "BusStopName" in stop
        }
...
def populate_route_info():
    ...
    routes = [{"route_name": key, "stops": routes[key]} for key in routes if key in good_routes]
