    ...
    if email:
        ...
        try:
            ...
            while len(confirmed_reservations) == 0 and i < libcal_search_span:
                ...
                confirmed_reservations = [
                    res
                    for res in libcal_reservations
                    if (
                        type(res) == dict
                        and res["status"] == "Confirmed"
                        and datetime.strptime(res["toDate"][:-6], "%Y-%m-%dT%H:%M:%S") >= now
                    )
                ]
                confirmed_reservations = [
                    res for res in confirmed_reservations if is_not_cancelled_in_db(res["bookId"])
                ]
                i += 1
        ...
        db_booking_ids = [
            str(x.booking_id)
            for x in db_bookings
            if x.end and x.end > now and not str(x.booking_id).isdigit() and not x.is_cancelled
        ]
        ...
        if missing_booking_ids:
            ...
            missing_reservations = studyspaces.get_reservations_for_booking_ids(
                missing_bookings_str
            )
            confirmed_missing_reservations = [
                res for res in missing_reservations if res["status"] == "Confirmed"
            ]
