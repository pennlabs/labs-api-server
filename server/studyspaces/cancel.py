...
def cancel_room():
    ...
    if booking:
        if (booking.user is not None) and (booking.user != user.id):
            return (
                jsonify({"error": "Unauthorized: This reservation was booked by someone else."}),
                400,
            )
    ...
    if booking_id.isdigit():
        ...
        try:
            ...
            if booking:
                ...
                save_booking(
                    lid=1, email=user.email, booking_id=booking_id, is_cancelled=True, user=user.id
                )
        ...
        if "error" not in resp:
            if booking:
                ...
                save_booking(
                    email=user.email, booking_id=booking_id, is_cancelled=True, user=user.id
                )
